...
import mesosphere.marathon.core.health.{ Health, HealthCheck, HttpHealthCheck }
...
class HealthCheckActorTest
  ...
  test("should not dispatch health checks for staging tasks") {
    ...
    val actor = f.actorWithLatch(HttpHealthCheck(maxConsecutiveFailures = 3, portIndex = Some(0)), latch)
  ...
  test("task should be killed if health check fails") {
    val f = new Fixture
    val actor = f.actor(HttpHealthCheck(maxConsecutiveFailures = 3, portIndex = Some(0)))
  ...
  class Fixture {
    ...
    def actorWithLatch(healthCheck: HealthCheck, latch: TestLatch) = TestActorRef[HealthCheckActor](
      Props(
        new HealthCheckActor(app, killService, HttpHealthCheck(portIndex = Some(0)), tracker, system.eventStream) {
